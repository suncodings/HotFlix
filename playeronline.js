class Moovie {
    constructor(options) {
        ...
        const defaults = {
            ...
            config: {
                ...
                i18n: {
                    play: "(پخش:توقف)",
                    mute: "(بیصدا:صدا دار)",
                    subtitles: "(فعال کردن:غیرفعال کردن) زیرنویس",
                    config: "تنظیمات",
                    fullscreen: "(ورود به حالت:خروج از حالت) تمام صفحه",
                    main_topic: "تنظیمات:",
                    main_caption: "زیرنویس",
                    main_offset: "سرعت زیرنویس",
                    main_speed: "سرعت",
                    main_disabled: "غیرفعال",
                    main_default: "پیش فرض",
                    caption_topic: "زیرنویس:",
                    caption_back: "برگشت",
                    caption_load: "آپلود زیرنویس",
                    offset_topic: "سرعت زیرنویس را تنظیم کنید",
                    speed_topic: "سرعت را تنظیم کنید"
                }
            },
            icons: {
                path: "https://raw.githubusercontent.com/suncodings/images/main/"
            },
            customEvents: [{
                type: "",
                to: "",
        ...
        var handleStorage = this.handleStorage = function handleStorage(order, variable, value) {
            switch (order) {
                ...
                case "get":
                    break;
                case "setStorage":
                    if (config["storage"]["captionOffset"]) { moovie_el_rinput.value = localStorage.getItem("captionOffset");
                        OffsetChange(); }
                    if (config["storage"]["playrateSpeed"]) { moovie_el_sinput.value = localStorage.getItem("playrateSpeed");
                        SpeedChange(); }
                        ...
                        if (parts[1] != undefined) {
                            ...
                            if (isNaN(parts[0])) { // If VTT Doesnt have cue number
                                ...
                                if (checkEmptyString) {
                                    ...
                                    if (CheckFormat == '.srt') { endtime = endtime.replace(/,/g, '.');
                                        starttime = starttime.replace(/,/g, '.'); }
        ...
        var SetupLogic = this.SetupLogic = function SetupLogic() {
            ...
            if (!detectTouchScreen()) {
                ...
                moovie_el_progress.addEventListener("change", function(event) { togglePlay();
                    togglePoster("hide"); }, false);

                // Hide div on mouse stop
                HideControls("close");
                var i = null;
                moovie_el_video.addEventListener('mousemove', e => { clearTimeout(i);
                    HideControls("open");
                    i = setTimeout(function() { HideControls("close"); }, 2000); });
                moovie_el_video.addEventListener('mouseleave', e => { clearTimeout(i);
                    HideControls("close"); });
                ...
                moovie_el_progress.addEventListener("touchmove", function(event) { Scrub(event);
                    video.pause(); });
                moovie_el_progress.addEventListener("change", function(event) { Scrub(event); if (!video.pause()) { togglePlay(); }
                    togglePoster("hide"); }, false);
                ...
                // Hide div on mouse stop
                var i = null;
                moovie_el_video.addEventListener('touchstart', e => { clearTimeout(i);
                    HideControls("open");
                    i = setTimeout(function() { HideControls("close"); }, 4000); });
                moovie_el_video.addEventListener('touchmove', e => { clearTimeout(i); });
                moovie_el_video.addEventListener('touchend', e => { clearTimeout(i);
                    i = setTimeout(function() { HideControls("close"); }, 4000); });
            }
        }
        ...
        var SetupPlayer = this.SetupPlayer = function SetupPlayer() {
            ...
            if (vposter != null) { moovie_el_poster.style.backgroundImage = "url(" + vposter + ")";
    ...
    // Set current time
    set currentTime(input) { this.video.currentTime = input;
            this.moovie_el_progressbar.value = input; }
        // Set volume
    set volume(input) { this.video.volume = input;
            this.moovie_el_volume.value = input;
            this.checkSoundLevel("checkMute"); }
        // Set speed
    set speed(input) { if (input < -0.1 || input > 8) { return "Value must be between -0.1 and 8"; } else { this.video.playbackRate = input;
                this.moovie_el_speedinput.value = input;
                this.SpeedChange(); } }
        // Set caption offset
    set captionOffset(input) { if (input < -5 || input > 5) { return "Value must be between -5 and 5"; } else { this.moovie_el_rangeinput.value = input;
                this.OffsetChange(); } }
        // Change source
    set source(input) { this.video.pause();
